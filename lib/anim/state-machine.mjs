...
export function createStateMachine(manifest, opts = {}) {
  ...
  let baseIdle = clips[idleCfg.base] ? idleCfg.base : Object.keys(clips)[0];
  const secondaryPool = (idleCfg.secondary || []).filter((c) => clips[c]);
  const boredClip = clips[idleCfg.boredClip] ? idleCfg.boredClip : null;
  // personality (C3) re-weights idle selection + timers. A RESOLVED object is passed in
  // (the caller reads personalities/*.json) so this module stays browser-safe (no node:).
  const pers = opts.personality || {};
  const sEveryScale = isFinitePositive(pers.secondaryEveryMsScale) ? pers.secondaryEveryMsScale : 1;
  const boredScale = isFinitePositive(pers.boredAfterMsScale) ? pers.boredAfterMsScale : 1;
  const secondaryWeights = (pers.secondaryWeights && typeof pers.secondaryWeights === 'object') ? pers.secondaryWeights : {};
  const boredAfterMs = (isFinitePositive(idleCfg.boredAfterMs) ? idleCfg.boredAfterMs : 30000) * boredScale;
  const secondaryEveryMs = (isFinitePositive(opts.secondaryEveryMs) ? opts.secondaryEveryMs : 9000) * sEveryScale;
  ...
  function pickWeighted(items) {
    ...
    const w = items.map((c) => { const x = secondaryWeights[c]; return typeof x === 'number' && x >= 0 ? x : 1; });
    const sum = w.reduce((a, b) => a + b, 0);
    if (sum <= 0) return items[Math.min(items.length - 1, Math.floor(rng() * items.length))];
    let r = rng() * sum;
    for (let i = 0; i < items.length; i++) { r -= w[i]; if (r <= 0) return items[i]; }
    return items[items.length - 1];
  }
  function playSecondary() {
    // weighted pick (personality bias), never the same one twice in a row (anti-repetition)
  ...
  /** release() — the thing driving the current reaction stopped (e.g. a tool finished);
   *  drop it and return to idle. No-op if already idling. (Recovery, never stuck.) */
  function release() { if (state && state.kind === 'reaction') { enterIdle(); return true; } return false; }

  /**
   * setIdleClip(clip) — change the base loop the pet idles on. Lets a sticky activity
   * mood swap the resting animation itself (e.g. a Codex pet's "running" processing loop
   * while the agent works, returning to "idle" when calm) — distinct from setIdleExpression,
   * which only changes the face worn over a fixed body loop. Re-enters idle immediately
   * if currently idling so the change is visible at once. No-op for an unknown clip.
   */
  function setIdleClip(clip) {
    if (!clip || !clips[clip] || clip === baseIdle) return false;
    baseIdle = clip;
    if (state && (state.kind === 'idle' || state.kind === 'secondary' || state.kind === 'bored')) enterIdle();
    return true;
  }
