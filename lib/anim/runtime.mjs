...
// C5 — optional SFX, SILENT unless explicitly enabled. Lazily fetches /assets/sfx/<key>.wav
// and plays via WebAudio with a per-mood pitch. A no-op (and never touches AudioContext)
// while disabled, so there are zero autoplay-policy surprises in the default (off) state.
  ...
  return {
    ...
    play(key, pitch = 1) {
      ...
      try {
        ...
        const got = cache[key] ? Promise.resolve(cache[key]) : fetch('/assets/sfx/' + key + '.wav').then((r) => r.arrayBuffer()).then((b) => ctx.decodeAudioData(b)).then((buf) => (cache[key] = buf));
        got.then((buf) => { const s = ctx.createBufferSource(); s.buffer = buf; s.playbackRate.value = pitch; const g = ctx.createGain(); g.gain.value = volume; s.connect(g); g.connect(ctx.destination); s.start(); }).catch(() => {});
...
export async function createRuntime(canvas, opts = {}) {
  ...
  // personality (C3) — a resolved object (the page fetches /personalities/<name>.json).
  // Inlined defaults keep this browser-safe (personality.mjs uses node:fs, can't import here).
  ...
  // accessibility: honour prefers-reduced-motion — DAMPEN movement, keep the information
  // (expression/palette/props still change; squash & particles are quieted, never faked away).
  ...
  function setMood(mood, ms) {
    ...
    const sk = { happy: 'happy', excited: 'excited', sad: 'sad', oops: 'oops', bashful: 'oops' }[S.mood];
    if (sk) sound.play(sk);
  }
  ...
  function addBird(label) { if (S.birds.length >= 5) return; S.birds.push({ id: nextBirdId++, label: label || 'task', born: S.lastT }); sound.play('bird'); }
  function removeBird() { const b = S.birds.shift(); if (b) poof(b._x || CX, b._y || 60); }
  function clearBirds() { S.birds = []; }
  function relief() { steam(CX - 14, GROUND - 40); steam(CX + 14, GROUND - 40); sm.react({ clip: 'react', expression: 'happy', priority: 6, return: 'idle' }); sound.play('relief'); }
  ...
  // a tool finished → drop its (looping) gag and return to idle
  function toIdle() { return sm.release(); }
  // C4 — slow mood drift (−1 stressed … +1 up): nudges reaction size + a cool palette floor
  ...
  function drawPet(dt) {
    ...
    // personality scales how BIG a reaction reads; mood drift nudges it (up = bigger);
    // reduced-motion dampens ALL movement (idle breathing included).
    ...
    // tired sheet from: high fullness, the sleepy face, OR a "stressed" mood drift (cool floor)
  ...
  return {
    ...
    get state() { const c = sm.current(); return { mood: S.mood, fullness: S.fullness, birds: S.birds.length, phase: S.phase, expression: c.expression, clip: c.clip, kind: c.kind, prop: c.prop, reduceMotion }; },
