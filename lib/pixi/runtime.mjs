...
export async function createPixiRuntime(canvas, opts = {}) {
  ...
  await app.init({ canvas, width: AW, height: AH, backgroundAlpha: 0, antialias: false, resolution: 1 });
  ...
  // ── display list (back → front): birds, body(calm/tired/error), prop, particles ──
  const birdLayer = new PIXI.Container(); app.stage.addChild(birdLayer);
  ...
  const S = { mood: 'idle', fullness: 0, phase: 'alive', moodLevel: 0, birds: [], fx: [], shake: 0, errFlash: 0, lastZ: 0, lastT: 0, blinking: false, blinkUntil: 0, nextBlinkAt: 0 };
  let nextBirdId = 1;

  function burst(x, y, n, pal) { if (reduceMotion) return; for (let i = 0; i < n; i++) { const a = (i / n) * Math.PI * 2; S.fx.push({ x, y, vx: Math.cos(a) * (1.2 + Math.random()), vy: Math.sin(a) * 1.2 - 0.6, life: 30 + Math.random() * 16, kind: 'dot', col: pal[i % pal.length] }); } }
  ...
  function setMood(mood) {
    ...
    if (S.mood === 'excited') { sm.react({ clip: 'react', expression: expr, priority: 6, return: 'idle' }); burst(CX, GROUND - 34, 14, ['#FEE761', '#F6757A', '#9BE86A', '#ffffff']); }
  ...
  function renderFx() {
    ...
    for (let i = S.fx.length - 1; i >= 0; i--) {
      const f = S.fx[i]; f.x += f.vx; f.y += f.vy; if (f.kind === 'steam') f.vx *= 0.96; f.life--;
      if (f.life <= 0) { S.fx.splice(i, 1); continue; }
      const sz = f.kind === 'steam' ? 3 : 2; const a = f.kind === 'steam' ? clamp(f.life / 50, 0, 0.8) : 1;
