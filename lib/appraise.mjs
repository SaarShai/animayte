...
/*
 * intensityDelta(text, valenceSign) → a small signed shift to apply to a family's base
 * arousal. SEVERE/AMPLIFY/EMPHATIC/DAMPEN read the raw heat either way; the negative-
 * SOFTENING cues (hedge / recovery / positive-lead) only fire for a NEGATIVE family —
 * "checking whether", "now restarting", and a "Good:" lead calm a setback, but must not
 * quiet a genuine win ("Fixed it!" stays a solid happy, not a faint content smile).
 * valenceSign defaults to -1 so a bare call still treats the text as a negative context.
 */
export function intensityDelta(rawText, valenceSign = -1) {
  if (!rawText) return 0;
  ...
  if (DAMPEN.test(text)) d -= 1;                       // "minor/small" softens either way
  if (valenceSign < 0) {                               // these only make sense for a setback
...
export function appraise(signal = {}, prev = {}) {
  ...
  if (signal.isError) {
    ...
    return makeSpec({ base: 'sad', cause, arousalDelta: roundDelta(intensityDelta(text, -1)), prev, reason: 'toolError', text: text || null });
  }
  ...
  if (signal.recentTexts != null) {
    ...
    for (const t of texts) {
      ...
      return makeSpec({
        ...
        emoji: s.emoji,
        arousalDelta: roundDelta(intensityDelta(t, sign(ax.valence))),
        prev,
        reason: 'text:' + s.mood,
        text: t,
      });
