...
- `animayte-gallery` — Drive the LIVE animayte pet through its ENTIRE reaction range on demand so the art team can watch the floating pet and sign off on the art
- `animayte-lint` — Validate an animayte pet's art manifest against the daemon's live command + reaction vocabulary
- `animayte-new-pet` — Scaffold a NEW animayte pet pack (a re-skin of the pet) against the real art contract
- `animayte-selftest` — Diagnose whether the animayte desktop pet is actually connected to THIS Claude Code session and reacting
