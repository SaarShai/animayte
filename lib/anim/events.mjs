...
export const TOOL_EVENTS = ['Reading', 'Searching', 'Writing', 'Running', 'Testing', 'Installing', 'Committing', 'Fetching', 'Planning'];
...
export function classifyTool(tool, input) {
  const t = String(tool || '');
  if (/^(TodoWrite|TodoRead|ExitPlanMode|UpdatePlan)$/.test(t)) return { event: 'Planning', category: 'plan' };
  if (/^(Read|NotebookRead|ReadFile)$/.test(t)) return { event: 'Reading', category: 'read' };
  if (/^(WebFetch|Fetch)$/.test(t)) return { event: 'Fetching', category: 'fetch' };
  if (/^(Grep|Glob|WebSearch|Search)$/.test(t)) return { event: 'Searching', category: 'search' };
