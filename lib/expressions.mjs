...
// ---- derived lookups -------------------------------------------------------
const stripVariation = (e) => e.replace(/[︀-️‍]/g, ''); // VS selectors / ZWJ
const EMOJI_TO_ID = {};
for (const ex of EXPRESSIONS) for (const em of ex.emojis) { EMOJI_TO_ID[em] = ex.id; EMOJI_TO_ID[stripVariation(em)] = ex.id; }
...
export function expressionForEmoji(emoji) { return EMOJI_TO_ID[emoji] || EMOJI_TO_ID[stripVariation(emoji)] || null; }
