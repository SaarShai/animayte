...
export async function createRuntime(canvas, opts = {}) {
  ...
  // ── mood tint (palette layer until B2 ships indexed swap on role-colored art) ──
  function tintFor() {
    if (S.errFlash > 0.02) return { col: '#E43B44', a: 0.34 * S.errFlash };
    const expr = MOOD_EXPRESSION[S.mood];
    if (expr === 'sleepy' || S.fullness > 0.6) return { col: '#3A6E8C', a: clamp(0.10 + S.fullness * 0.28, 0, 0.34) };
    return null;
  }
  ...
  function drawPet(dt) {
    ...
    const drawW = cell * sx, drawH = cell * sy;
    ...
    const col = cur.frame % cols;
    ctx.drawImage(sheet, col * cell, row * cell, cell, cell, dx, dy, Math.round(drawW), Math.round(drawH));
    // tint only the pet pixels (nothing else drawn yet this frame)
    const tint = tintFor();
    if (tint) { ctx.save(); ctx.globalCompositeOperation = 'source-atop'; ctx.globalAlpha = tint.a; ctx.fillStyle = tint.col; ctx.fillRect(dx - 2, dy - 2, Math.round(drawW) + 4, Math.round(drawH) + 4); ctx.restore(); }
  }
  function drawBirds() {
    if (!bird) return;
  ...
  return {
    setMood, setFullness, addBird, removeBird, clearBirds, relief, sleep, wake, reset,
    get state() { return { mood: S.mood, fullness: S.fullness, birds: S.birds.length, phase: S.phase, expression: sm.current().expression, clip: sm.current().clip, kind: sm.current().kind }; },
