...
// the agent's tool ACTIVITY (a classifyTool event, lib/anim/events.mjs) → the ITEM to
// show. This is the second half of the "what's it doing" bridge: emoji says it in words
// (EMOJI_ITEM), the live tool stream says it in actions. Both resolve to the same prop
// vocabulary so appraise() is the daemon's only "what to express" call. No new sprites —
// every value is already in ITEMS.
export const EVENT_ITEM = {
  Reading: 'book', Searching: 'magnifier', Fetching: 'globe', Planning: 'lightbulb',
  Writing: 'hammer', Running: 'terminal', Testing: 'magnifier',
  Installing: 'box', Committing: 'box',
  Asking: 'question', Waiting: 'question',
};
export const itemForEvent = (event) => EVENT_ITEM[event] || null;
